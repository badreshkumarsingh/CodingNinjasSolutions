...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) {
    try {
      ...
      // 1. Remove old tag
      await collection.updateOne({_id: new ObjectId(id)}, {$pull: {tags: oldTag}});

      // 2. Add new tag
      await collection.updateOne({_id: new ObjectId(id)}, {$push: {tags: newTag}});
      return;
    } catch(err) {
      throw err;
    }
  ...
  async deleteTagFromExpense(id, tag) {
    ...
    await collection.updateOne({_id: new ObjectId(id)}, {$pull: {tags: tag}});
    return;
  }
}
